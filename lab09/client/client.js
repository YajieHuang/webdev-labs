...
const handleServerMessage = ev => {
    ...
    if (data.type === "login") {
        ...
        for(var i = 0; i < data.active_users.length; i ++){
            qs("#users-list").innerHTML += `<li> ${data.active_users[i]}</li>`;
        }
        ...
        console.log(data);
        qs("#chat").setAttribute('aria-live', 'off')
        if(data.username == username){
            qs("#chat").innerHTML += `<li class="right" aria-live="off"> ${"YOU: " + data.text} </li>`;
        }
        else{
            qs("#chat").innerHTML += `<li class="left" aria-live="off"> ${data.username + ": " + data.text} </li>`;
        }
